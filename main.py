...
# -------------------------------
# Defect Detection: Light & High Contrast
# -------------------------------
def detect_defects_and_annotate(image):
    ...
    # Estimate background using heavy blur
    ...
    # Adaptive threshold to catch subtle light defects
    # Low threshold for low-contrast areas
    _, low_thresh = cv2.threshold(diff, 15, 255, cv2.THRESH_BINARY)

    # High threshold for high-contrast areas
    _, high_thresh = cv2.threshold(diff, 35, 255, cv2.THRESH_BINARY)

    # Combine low and high contrast masks
    combined = cv2.bitwise_or(low_thresh, high_thresh)
